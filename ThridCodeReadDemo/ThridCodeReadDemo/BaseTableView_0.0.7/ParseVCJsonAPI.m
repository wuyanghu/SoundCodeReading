...
+ (BaseSectionsModel *)parseServerJsonToBaseSetcionModel:(NSDictionary *)jsonDict{
    ...
    BaseSectionsModel * sectionModel = [self parseLocalJsonToBaseSetcionModel:configDict];
    return sectionModel;
}

+ (BaseSectionsModel *)parseLocalJsonToBaseSetcionModel:(NSDictionary *)configDict{
    BaseSectionsModel * sectionsModel = [BaseSectionsModel new];
    ...
    for (int i = 0;i<contentArray.count;i++) {
        ...
        BaseSectionModel * sectionModel = [BaseSectionModel new];
        sectionModel.sectionTitle = titleArray[i];
        
        for (id content in sectionArr) {
            BaseRowModel * rowModel = [BaseRowModel new];
