...
- (void)configImage:(UIImage *)image smallImageUrl:(NSString *)smallImageUrl imageUrl:(NSString *)imageUrl placeholderImage:(UIImage *)placeholderImage scale:(CGFloat)scale
{
    ...
    if (image) {
        ...
        UIImage *image = [manager.imageCache imageFromDiskCacheForKey:imageUrl];
        if (image) {
            [self setImage:image scale:scale];
        } else {
            if (smallImageUrl && smallImageUrl.length > 0) {
                //展示缩略图
                image = [manager.imageCache imageFromDiskCacheForKey:smallImageUrl];
                if (image) {
                    [self setImage:image scale:1];
                }
            }
            //下载
            [self showAnimation];
            
            self.imageDownloadOperation = [[SDWebImageManager sharedManager]downloadImageWithURL:[NSURL URLWithString:imageUrl] options:SDWebImageRetryFailed progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    self.indexPath = [NSIndexPath indexPathForRow:page inSection:0];
}
...
- (UIImageView *)imageViewForDismissView {
    CMPhotoCell *cell = (CMPhotoCell *)[_collectionView cellForItemAtIndexPath:self.indexPath];
