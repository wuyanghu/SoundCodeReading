...
static uint32_t __CFSendTrivialMachMessage(mach_port_t port, uint32_t msg_id, CFOptionFlags options, uint32_t timeout) {
    ...
    result = mach_msg(&header, MACH_SEND_MSG|options, header.msgh_size, 0, MACH_PORT_NULL, timeout, MACH_PORT_NULL);//发送系统消息?
...
//检查CFRunLoopRef是否为空,commonModes是否包含name,sources0、sources1、timers是否为空，
...
/* Bit 1 of the base reserved bits is used for signalled state 基本保留位的第1位用于表示信号状态*/
//获取信号状态
...
struct __CFRunLoopTimer {
    ...
    //存放所有包含该timer的 mode的 modeName，意味着一个timer可能会在多个mode中存在
...
#pragma mark - 获取当前线程具体函数
// should only be called by Foundation
// t==0 is a synonym for "main thread" that always works
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    if (pthread_equal(t, kNilPthreadT)) {//如果t为空，则设置为主线程
        t = pthread_main_thread_np();
    }
    __CFLock(&loopsLock);//加锁访问
    
    //__CFRunLoops为空，则设置主线程
    if (!__CFRunLoops) {
...
#pragma mark - 返回当前RunLoop的所有mode
...
void CFRunLoopAddCommonMode(CFRunLoopRef rl, CFStringRef modeName) {
    ...
    if (!CFSetContainsValue(rl->_commonModes, modeName)) {
        ...
        if (NULL != set) {
            ...
            /* add all common-modes items to new mode */ //__CFRunLoopAddItemsToCommonMode是一个方法，里面会调用CFRunLoopAddSource/CFRunLoopAddObserver/CFRunLoopAddTimer
...
#pragma mark - 通知观察者回调函数
//OBSERVER
static void __CFRUNLOOP_IS_CALLING_OUT_TO_AN_OBSERVER_CALLBACK_FUNCTION__() __attribute__((noinline));
static void __CFRUNLOOP_IS_CALLING_OUT_TO_AN_OBSERVER_CALLBACK_FUNCTION__(CFRunLoopObserverCallBack func, CFRunLoopObserverRef observer, CFRunLoopActivity activity, void *info) {
    if (func) {
        func(observer, activity, info);//回调函数
    }
    asm __volatile__(""); // thwart tail-call optimization
}
//TIMER
...
//while循环执行rl的block任务
//遍历rl中的model,与curModel _name相同时，执行block任务
static Boolean __CFRunLoopDoBlocks(CFRunLoopRef rl, CFRunLoopModeRef rlm) { // Call with rl and rlm locked
    ...
    while (item) {//遍历链表
        ...
        if (doit) {
            ...
            void (^block)(void) = curr->_block;//执行curr的block任务
            CFRelease(curr->_mode);
            free(curr);
            if (doit) {
                //执行block任务
...
/*
 ...
 CFRunLoopActivity activity：runloop的六种状态
获取监听当前所有runloop的集合，遍历collectedObservers调用观察者函数
 */
static void __CFRunLoopDoObservers() __attribute__((noinline));
static void __CFRunLoopDoObservers(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFRunLoopActivity activity) {	/* DOES CALLOUT */
    CHECK_FOR_FORK();

    CFIndex cnt = rlm->_observers ? CFArrayGetCount(rlm->_observers) : 0;//获取model观察者的count
    if (cnt < 1) return;

    /* Fire the observers */
    STACK_BUFFER_DECL(CFRunLoopObserverRef, buffer, (cnt <= 1024) ? cnt : 1);
    CFRunLoopObserverRef *collectedObservers = (cnt <= 1024) ? buffer : (CFRunLoopObserverRef *)malloc(cnt * sizeof(CFRunLoopObserverRef));//cnt<1024取缓存，否则分配cnt大小内存
    CFIndex obs_cnt = 0;
    ...
    for (CFIndex idx = 0; idx < obs_cnt; idx++) {
        ...
        if (__CFIsValid(rlo)) {
            ...
            __CFRunLoopObserverUnlock(rlo);
            __CFRUNLOOP_IS_CALLING_OUT_TO_AN_OBSERVER_CALLBACK_FUNCTION__(rlo->_callout, rlo, activity, rlo->_context.info);//通知观察者，回调函数
            if (doInvalidate) {
...
static Boolean __CFRunLoopDoSources0(CFRunLoopRef rl, CFRunLoopModeRef rlm, Boolean stopAfterHandle) {	/* DOES CALLOUT */
    ...
    if (NULL != sources) {
        ...
        if (CFGetTypeID(sources) == CFRunLoopSourceGetTypeID()) {
            ...
            __CFRunLoopSourceLock(rls);
            if (__CFRunLoopSourceIsSignaled(rls)) {//判断Signaled
                __CFRunLoopSourceUnsetSignaled(rls);//设置Signaled为0
                if (__CFIsValid(rls)) {
                    __CFRunLoopSourceUnlock(rls);
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    if (seconds <= 0.0) { // instant timeout
        ...
        uint64_t ns_at = (uint64_t)((__CFTSRToTimeInterval(startTSR) + seconds) * 1000000000ULL);//纳秒?
    ...
    __CFRunLoopUnsetIgnoreWakeUps(rl);//设置IgnoreWakeUps标识为0,设置RunLoop为可以被唤醒状态
    ...
    if (MACH_PORT_NULL != dispatchPort && !didDispatchPortLastTime) {
        ...
        if (__CFRunLoopServiceMachPort(dispatchPort,// 监听的端口
                                       ...
                                       0,// 超时时间，这里为 0 表示不会睡眠
                                       &voucherState, NULL)) {
            goto handle_msg;//如果接收到了消息的话，前往第9步开始处理msg
        }
#elif DEPLOYMENT_TARGET_WINDOWS
    ...
    __CFRunLoopSetSleeping(rl);//睡眠,仅设置标志位?
    __CFPortSetInsert(dispatchPort, waitSet);//内核了解下?
...
//进入此循环后，线程进入休眠，直到收到新消息才跳出该循环，继续执行run loop
    ...
    __CFRunLoopWaitForMultipleObjects(waitSet, NULL, poll ? 0 : TIMEOUT_INFINITY, rlm->_msgQMask, &livePort, &windowsMessageReceived);
        ...
        __CFRunLoopSetIgnoreWakeUps(rl);//IgnoreWakeUps为1

        // user callouts now OK again
	__CFRunLoopUnsetSleeping(rl);//Sleeping为0
        // 8. 通知 Observers: RunLoop 的线程刚刚被唤醒了。
	if (!poll && (rlm->_observerMask & kCFRunLoopAfterWaiting)) __CFRunLoopDoObservers(rl, rlm, kCFRunLoopAfterWaiting);
        
        // 9.处理收到的消息
    handle_msg:;
        __CFRunLoopSetIgnoreWakeUps(rl);//IgnoreWakeUps为0
        ...
        if (MACH_PORT_NULL == livePort) {
            CFRUNLOOP_WAKEUP_FOR_NOTHING();//通过CFRunloopWake唤醒
            // handle nothing
            ...
            ResetEvent(rl->_wakeUpPort);//继续运行
#endif
        }
#if USE_DISPATCH_SOURCE_FOR_TIMERS
        //如果是定时器事件
        ...
        //如果是dispatch到main queue的block
            ...
            void *msg = 0;
#endif
            // 9.2 如果有dispatch_get_main_queue的block，执行block。
        ...
        if (sourceHandledThisLoop && stopAfterHandle) {
            ...
            retVal = kCFRunLoopRunTimedOut;// 超出传入参数标记的超时时间了
        } else if (__CFRunLoopIsStopped(rl)) {
                __CFRunLoopUnsetStopped(rl);
            retVal = kCFRunLoopRunStopped;// 被外部调用者强制停止了
        } else if (rlm->_stopped) {
            rlm->_stopped = false;
            retVal = kCFRunLoopRunStopped;
        } else if (__CFRunLoopModeIsEmpty(rl, rlm, previousMode)) {
            retVal = kCFRunLoopRunFinished;// source/timer/observer一个都没有了
        }
...
#pragma mark - RunLoop入口
/*
 CFRunLoopRef rl:CFRunLoop对象
 */
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    CHECK_FOR_FORK();
    if (__CFRunLoopIsDeallocating(rl)) return kCFRunLoopRunFinished;//rl已经dealloc，return finished
    __CFRunLoopLock(rl);//加锁
    
    CFRunLoopModeRef currentMode = __CFRunLoopFindMode(rl, modeName, false);// 在rl中找到modeName正在运行的model
    // 如果mode里没有source/timer/observer, 直接返回。
    if (NULL == currentMode || __CFRunLoopModeIsEmpty(rl, currentMode, rl->_currentMode)) {
        Boolean did = false;//?did干啥的，写死的局部变量？
	...
	if (currentMode->_observerMask & kCFRunLoopEntry ) __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopEntry);//进入Entry
    
	result = __CFRunLoopRun(rl, currentMode, seconds, returnAfterSourceHandled, previousMode);//run do...while循环
...
void CFRunLoopAddSource(CFRunLoopRef rl, CFRunLoopSourceRef rls, CFStringRef modeName) {	/* DOES CALLOUT */
    ...
    if (modeName == kCFRunLoopCommonModes) {
        ...
        /*
         以上分支的逻辑就是，如果你往kCFRunLoopCommonModes里面添加一个source，
         那么所有_commonModes里的mode都会添加这个source
         */
        ...
        //rlm中的_sources0和_sources1都不包含rls
